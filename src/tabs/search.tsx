...
import { Search, ExternalLink, Globe, ArrowRight, Layout, Info, Loader2, BookPlus, Settings } from "lucide-react"
...
export default function SearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // 并发搜索所有内置书源
      const searchPromises = BUILTIN_RULES.map(async (rule) => {
        const engine = new ScraperEngine(rule)
        try {
          return await engine.search(query)
        } catch (e) {
          console.error(`Search failed for ${rule.name}:`, e)
          return []
        }
      })
  ...
  const handleAddToShelf = async (result: ScraperSearchResult) => {
    const rule = BUILTIN_RULES.find(r => r.id === result.sourceId)
    if (!rule) return
    ...
    try {
      ...
      // 避免重复添加
      if (bookshelf.some(b => b.title === info.bookName && b.author === info.author)) {
      ...
      toast.success(`《${info.bookName}》已成功入库，共 ${toc.length} 章`)
    } catch (error) {
    ...
    <ThemeProvider attribute="class" defaultTheme="system" enableSystem>
      ...
      <div className="min-h-screen bg-background text-foreground font-sans p-8 max-w-6xl mx-auto">
        ...
        <header className="flex items-center justify-between mb-8 border-b pb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">一站式发现全网优质文学资源</p>
            </div>
          </div>
        ...
        <div className="mb-10 max-w-2xl mx-auto text-center space-y-4">
          <h2 className="text-3xl font-black tracking-tight">找小说，一个搜索框就够了</h2>
          <div className="relative group">
            ...
            <Button 
              ...
              disabled={isLoading}
            >
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "全网搜"}
            </Button>
          </div>
        ...
        {hasSearched && (
          <section className="mb-12 space-y-6">
            <div className="flex items-center justify-between border-b pb-4">
              ...
              {isLoading && <span className="text-xs text-muted-foreground animate-pulse">正在从多个书源抓取中...</span>}
            </div>

            {results.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {results.map((result, i) => (
                  <div key={`${result.sourceId}-${i}`} className="p-4 rounded-2xl border bg-card hover:border-primary/30 transition-all flex flex-col gap-3 group">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h3 className="font-bold text-lg line-clamp-1 group-hover:text-primary transition-colors">{result.bookName}</h3>
                        <p className="text-sm text-muted-foreground">{result.author || "未知作者"}</p>
                      </div>
                    ...
                    <div className="text-[11px] text-muted-foreground space-y-1">
                      {result.latestChapter && <p className="line-clamp-1">最新：{result.latestChapter}</p>}
                      {result.lastUpdateTime && <p>更新：{result.lastUpdateTime}</p>}
                    </div>
                    ...
                    <div className="mt-auto flex gap-2">
                       ...
                       <Button size="icon" variant="outline" className="w-9 h-9 rounded-xl shrink-0" asChild>
              ...
              <div className="py-20 text-center space-y-4">
                ...
                <p className="text-muted-foreground">没有找到匹配的结果，请换个词试试</p>
              </div>
            )}
          </section>
        )}
          ...
          <div className="md:col-span-2 space-y-8">
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded">推荐收藏</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { name: "知宣搜书", url: "https://zhixuan.la", desc: "主打极致简洁，提供高质量 EPUB 资源下载。", tag: "热门推荐" },
                  { name: "鸠摩搜索", url: "https://www.jiumodiary.com", desc: "非常强大的文档搜索引擎，资源极其丰富。", tag: "必备工具" },
                  { name: "我的小书屋", url: "http://mebook.cc", desc: "老牌精选电子书分享站点，带详细内容介绍。", tag: "精选周刊" },
                  { name: "书伴 (Bookfere)", url: "https://bookfere.com", desc: "不仅是资源，更是关于阅读的一切资讯。", tag: "知识库" },
                  { name: "田间漫步", url: "https://www.tjmb.cc", desc: "精美排版的小说资源站。", tag: "新晋黑马" },
                  { name: "搬书匠", url: "http://www.banshujiang.com", desc: "如果你也对编程和技术书感兴趣。", tag: "技术领域" }
                ].map(site => (
                  <a 
                    key={site.name} 
                    href={site.url} 
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-bold text-lg group-hover:text-primary transition-colors">{site.name}</div>
                    ...
                    <div className="text-sm text-muted-foreground leading-snug mb-4">{site.desc}</div>
          ...
          <aside className="space-y-6">
            <div className="rounded-2xl border border-dashed border-primary/40 p-6 bg-primary/5 relative overflow-hidden">
               <div className="relative z-10">
                 ...
                 <h3 className="font-bold text-lg mb-2 text-primary">内置爬虫已开启</h3>
                 <p className="text-sm text-muted-foreground mb-4">
                   直接在上方搜索框输入关键词，即可一键将书籍抓取到扩展书架中，无需手动下载文件。
                 </p>
                 <Badge variant="secondary" className="bg-primary/10 text-primary border-none">核心功能已上线</Badge>
               </div>
            </div>
