...
import { BookOpen, ChevronLeft, ChevronRight, List, X, Home } from "lucide-react"
          ...
          <header className="flex items-center justify-between mb-8 border-b pb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">沉浸式阅读体验</p>
              </div>
            </div>
          ...
          <div className="rounded-2xl border bg-card p-8 mb-8">
            <div className="flex gap-8 items-start">
              ...
              <div className="flex-1">
                ...
                <p className="text-lg text-muted-foreground mb-6">{book.author || "未知作者"}</p>

                {/* 统计信息 */}
                <div className="grid grid-cols-3 gap-4 mb-8 p-4 bg-muted/30 rounded-lg">
                  <div>
                    <div className="text-sm text-muted-foreground mb-1">总章节</div>
                    <div className="text-2xl font-bold">{chapters.length}</div>
                  </div>
                  <div>
                    <div className="text-sm text-muted-foreground mb-1">当前进度</div>
                    <div className="text-2xl font-bold">第 {currentChapterIndex + 1} 章</div>
                  </div>
                  <div>
                    <div className="text-sm text-muted-foreground mb-1">完成度</div>
                    <div className="text-2xl font-bold">{Math.round(((currentChapterIndex + 1) / chapters.length) * 100)}%</div>
                  </div>
                </div>
            ...
            <div className="flex items-center justify-between p-4">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div className="text-sm min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground line-clamp-1">{currentChapter.title}</div>
                </div>
              </div>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <article className="max-w-3xl mx-auto px-6 py-8 sm:px-8">
              ...
              <div className="max-w-none leading-relaxed">
                {currentChapter.content && currentChapter.content.includes("<") ? (
                  <div 
                  ...
                  <div className="space-y-4">
                    {currentChapter.content && currentChapter.content.split("\n").map((paragraph, idx) => (
                      paragraph.trim() && (
                        <p key={idx} className="text-base leading-8 text-justify">
                          {paragraph}
                        </p>
                      )
                    ))}
...
function sanitizeHtml(html: string): string {
  const allowedTags = ["p", "br", "strong", "em", "u", "h1", "h2", "h3", "h4", "h5", "h6", "div", "span", "img", "a"]
