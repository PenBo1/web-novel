...
export class ScraperEngine {
  ...
  async fetchHtml(
    ...
    // 渲染层日志：确认消息是否发出
    console.log(
      `[Engine] 发送请求到后台: ${method.toUpperCase()} ${absoluteUrl}`,
    );
  ...
  async search(keyword: string): Promise<SearchResult[]> {
    ...
    try {
      if (searchRule.method === "post") {
        ...
        const encodedData = searchRule.data?.replace(
          "%s",
    ...
    if (!html || html.length === 0) {
      console.error(`[Engine] 收到空的 HTML 响应`);
      return [];
    }
    ...
    // 检查是否是错误页面
    if (
      doc.documentElement.innerHTML.includes("404") ||
      doc.documentElement.innerHTML.includes("error")
    ) {
      console.warn(`[Engine] 可能收到错误页面`);
    }

    const items = doc.querySelectorAll(searchRule.result);
    console.log(
      `[Engine] 搜索 "${keyword}" 在 ${this.rule.name}，找到 ${items.length} 个项目`,
    );
    ...
    items.forEach((item, index) => {
      try {
        ...
        if (!bookName || !bookName.trim()) {
          console.warn(`[Engine] 项目 ${index}: 书名为空`);
          filteredCount++;
          return;
        }
        ...
        if (!bookUrl || !bookUrl.trim()) {
          console.warn(
            `[Engine] 项目 ${index}: 书籍 URL 为空，书名: ${bookName}`,
          );
          filteredCount++;
          return;
        }
        ...
        try {
          const absoluteUrl = new URL(bookUrl, this.rule.url).href;
          results.push({
            sourceId: this.rule.id,
            bookName: bookName.trim(),
            url: absoluteUrl,
            author: author?.trim() || "未知作者",
            latestChapter: searchRule.latestChapter
              ? this.parseContent(item, searchRule.latestChapter)
              : undefined,
            lastUpdateTime: searchRule.lastUpdateTime
              ? this.parseContent(item, searchRule.lastUpdateTime)
              : undefined,
            category: searchRule.category
              ? this.parseContent(item, searchRule.category)
              : undefined,
            status: searchRule.status
              ? this.parseContent(item, searchRule.status)
              : undefined,
            wordCount: searchRule.wordCount
              ? this.parseContent(item, searchRule.wordCount)
              : undefined,
          });
        } catch (urlError) {
          console.error(
            `[Engine] 项目 ${index}: URL 解析失败 - ${bookUrl}`,
            urlError,
          );
          filteredCount++;
        }
      } catch (e) {
        console.error(`[Engine] 项目 ${index}: 解析失败`, e);
        filteredCount++;
      }
    });

    console.log(
      `[Engine] 搜索完成: 找到 ${items.length} 个项目，成功 ${results.length} 个，过滤 ${filteredCount} 个`,
    );
  ...
  async getBookInfo(
    bookUrl: string,
  ): Promise<{ info: BookInfo; toc: Chapter[] }> {
    ...
    const info: BookInfo = {
      ...
      coverUrl: bookRule.coverUrl
        ? this.parseContent(doc, bookRule.coverUrl, "attr", "src") ||
          this.parseContent(doc, bookRule.coverUrl, "attr", "content")
        : undefined,
      category: bookRule.category
        ? this.parseContent(doc, bookRule.category)
        : undefined,
      latestChapter: bookRule.latestChapter
        ? this.parseContent(doc, bookRule.latestChapter)
        : undefined,
      lastUpdateTime: bookRule.lastUpdateTime
        ? this.parseContent(doc, bookRule.lastUpdateTime)
        : undefined,
    };
    ...
    if (this.rule.toc.url) {
      const bookIdMatch =
        bookUrl.match(/\/(\d+)\/?$/) ||
        bookUrl.split("/").filter(Boolean).pop()?.match(/(\d+)/);
    ...
    if (this.rule.toc.list) {
      const cleanHtml = this.parseContent(
        tocDoc.documentElement.innerHTML,
        this.rule.toc.list,
      );
      if (cleanHtml) {
