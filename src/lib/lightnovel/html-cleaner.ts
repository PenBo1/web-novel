...
export const replaceImageSrc = (element: Element): void => {
  ...
  images.forEach(img => {
    ...
    // 移除无效属性，只保留必要的属性
    const validAttrs = ["alt", "class", "dir", "height", "id", "ismap", "lang", "longdesc", "style", "title", "usemap", "width", "src", "xml:lang"]
...
export const extractChapterContent = (
  ...
  if (source === "bili") {
    ...
    contentElement = doc.querySelector("#acontent") || doc.querySelector(".bcontent")
    if (contentElement) {
