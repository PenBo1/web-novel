// 哔哩轻小说解析器 - 完整实现参考 bili_novel_packer
...
export const fetchBiliNovelInfo = async (novelId: string) => {
  ...
  console.log(`[BiliParser] Fetching info from: ${url}`);
  ...
  const html = await scheduler.run(async () => {
    ...
    if (!response.ok) {
      console.error(
        `[BiliParser] Failed to fetch info: ${response.status} ${response.statusText}`,
      );
  ...
  console.log(`[BiliParser] Parsed title: ${title}`);
...
export const fetchBiliCatalog = async (novelId: string): Promise<Volume[]> => {
  ...
  console.log(`[BiliParser] Fetching catalog from: ${url}`);
  ...
  const html = await scheduler.run(async () => {
    ...
    if (!response.ok) {
      console.error(
        `[BiliParser] Failed to fetch catalog: ${response.status} ${response.statusText}`,
      );
...
/**
 * 获取 shuffle 参数（用于解密打乱的段落）
 */
const getShuffleParams = (
  doc: Document,
): {
  fixedLength: number;
  seed: number;
  ...
  const chapterIdMatch =
    doc.documentElement.outerHTML.match(/chapterid:'(\d+)'/);
...
const shuffleArray = (
  arr: number[],
  params: { seed: number; a: number; c: number; mod: number },
): void => {
...
const shuffleContent = (
  ...
  const pElements = Array.from(content.querySelectorAll("p")).filter((p) =>
    p.textContent?.trim(),
  );
...
const getChapterPage = async (
  ...
  const html = await scheduler.run(async (controller) => {
    ...
    // 检测 Cloudflare 限制
    if (
      text.includes("Cloudflare to restrict access") ||
      text.includes("503 Service Temporarily Unavailable")
    ) {
      console.warn("检测到 Cloudflare 限制，暂停 10 秒后重试...");
      controller.pause();
      await new Promise((resolve) => setTimeout(resolve, 10000));
      controller.resume();
      throw new Error("Cloudflare rate limit detected, retrying...");
    }
  ...
  const urlMatch = doc.documentElement.outerHTML.match(
  ...
  if (
    prevLink &&
    (prevLink.textContent === "上一页" || prevLink.textContent === "上一頁") &&
    prevUrl
  ) {
  ...
  if (
    nextLink &&
    (nextLink.textContent === "下一页" || nextLink.textContent === "下一頁") &&
    nextUrl
  ) {
...
export const parseBilibiliNovel = async (input: string): Promise<NovelInfo> => {
  console.log(`[BiliParser] Parsing input: ${input}`);
  const novelId = extractBiliNovelId(input);
  console.log(`[BiliParser] Extracted ID: ${novelId}`);
  ...
  console.log(`[BiliParser] Found ${volumes.length} volumes`);
 ...
 * 获取章节内容（用于下载）
 */
export const getBiliChapterContent = async (
  chapter: Chapter,
): Promise<string> => {
 ...
 * 下载图片
 */
export const getBiliImage = async (src: string): Promise<Uint8Array> => {
  return imageScheduler.run(async () => {
    let imageSrc = src;
