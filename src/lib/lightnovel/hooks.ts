...
import { downloadNovelChapters } from "./downloader";
...
export const useNovelParser = () => {
  ...
  const parseNovel = async (
    ...
    try {
      const novelInfo =
        source === "bili"
          ? await parseBilibiliNovel(input)
          : await parseWenkuNovel(input);
...
export const useNovelDownloader = () => {
  ...
  const downloadNovel = async (
    ...
    try {
      ...
      toast.success(
        `《${novelInfo.title}》已添加到书架，共 ${chapters.length} 章`,
      );
    } catch (error: any) {
...
export const useVolumeSelection = (initialVolumes: number = 0) => {
  ...
  const toggleAllVolumes = (totalVolumes: number) => {
    if (selectedVolumes.size === totalVolumes) {
      ...
      setSelectedVolumes(
        new Set(Array.from({ length: totalVolumes }, (_, i) => i)),
      );
    }
  };
