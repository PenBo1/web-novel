...
function OptionsBody() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // 冲突检测
      const conflict = shortcuts.find(s => s.id !== recordingId && JSON.stringify(s.keys) === JSON.stringify(keys))
      if (conflict) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans p-8 max-w-3xl mx-auto">
      ...
      <div className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
               <div className="space-y-0.5">
                 ...
                 <p className="text-xs text-muted-foreground">切换扩展弹出页面的明暗外观</p>
               </div>
               <Switch
            ...
            <div className="flex items-center justify-between">
               <div className="space-y-0.5">
                 ...
                 <p className="text-xs text-muted-foreground">切换网页底部阅读条的明暗外观</p>
               </div>
               <Switch
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">打开新网页时是否自动显示阅读条</p>
              </div>
              <Switch checked={settings.defaultShow} onCheckedChange={(c) => saveSettings({ ...settings, defaultShow: c })} />
            </div>

            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label className="text-base">阅读条位置</Label>
                <p className="text-xs text-muted-foreground">选择阅读条固定在屏幕顶部还是底部</p>
              </div>
              <div className="flex items-center gap-2">
                  ...
                  <Button 
                    variant={settings.position !== "top" ? "default" : "outline"} 
                    size="sm"
                    onClick={() => saveSettings({ ...settings, position: "bottom" })}
                  >底部</Button>
              </div>
            </div>
          </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold border-b pb-2 flex-1">快捷键设置</h2>
            <Button 
              variant="outline" 
              size="sm" 
            ...
            {shortcuts.map(item => (
              <div key={item.id} className="flex items-center justify-between py-2">
                ...
                <div className="flex gap-3 items-center">
                  <Kbd className="text-xs min-w-[80px] justify-center">{item.keys.join(" + ") || "未设置"}</Kbd>
                  <Button 
